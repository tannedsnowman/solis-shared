/**
 * BACKUP PORT, in the phase-table style.
 *
 * Moved off the crowded Device tab into a panel of its own so it can show
 * all three phases. Unlike the grid port, the backup (EPS) port publishes S
 * per phase directly (33523/33526/33529), so nothing in `backupPortTable`
 * is calculated -- see its doc comment in `phaseTable.ts`.
 */
import React, { useEffect } from 'react';
import PhaseTableView from '../data/PhaseTableView';
import type { RawReader } from '../data/rawReader';
import { backupPortTable } from '../data/phaseTable';

interface BackupPortPanelProps {
  /**
...
const BackupPortPanel: React.FC<BackupPortPanelProps> = ({ onAddresses, reader }) => {
  const model = backupPortTable(reader);

  useEffect(() => {
    onAddresses?.(model.addresses);
    // The address list is fixed by which registers backupPortTable reads, so
    // it only needs reporting when its contents change, not on every poll
    // tick.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [model.addresses.join(',')]);

  return <PhaseTableView title="BACKUP PORT" model={model} />;
};

export default BackupPortPanel;
