/**
 * SMART LOAD, in the phase-table style.
 *
 * A controllable load wired into the Smart port. Shares the port's own
 * V/I/per-phase-P registers with the generator and AC-coupled roles (see
...
import { smartLoadTable } from '../data/phaseTable';

interface SmartLoadPanelProps {
  /**
...
const SmartLoadPanel: React.FC<SmartLoadPanelProps> = ({ onAddresses, reader }) => {
  const model = smartLoadTable(reader);

  useEffect(() => {
    onAddresses?.(model.addresses);
    // The address list is fixed by which registers smartLoadTable reads, so
    // it only needs reporting when its contents change, not on every poll
    // tick.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [model.addresses.join(',')]);

  return <PhaseTableView title="SMART LOAD" model={model} />;
};

export default SmartLoadPanel;
